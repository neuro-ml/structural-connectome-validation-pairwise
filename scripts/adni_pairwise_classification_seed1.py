...
pairs_data = pd.read_csv('../data/adni_pairs_data_with_dx_group_without_isolated_nodes.csv', index_col = None)
...
class DataTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, pairs_data, data_dir='../data/adni/matrices/'):
        matrices = {}
        if self.target != 'All':
        ...
        pairs_data = generate_even_sample(pairs_data, n = int(pairs_data.are_same.sum()), seed = 1)
...
targets = ['Normal', 'EMCI', 'LMCI', 'AD', 'All']
steps = Steps(steps)
...
output_folder = 'ADNI_pairwise_results/'
...
for target in targets:
    ...
    pp.get_results( X, y.values,
                    featuring_steps = [ 'norm',
                                        'base_features',
                                        'pairwise_features' ],
                    results_file    = output_folder + target + '_seed1.csv' )
    print('\n\n', target, 'Done')
